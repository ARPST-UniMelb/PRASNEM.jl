function createGenerators(generator_input_file, timestep_generator_input_file, units, regions_selected, start_dt, end_dt; 
    ...
    for i in 1:nrow(gen_info)
        ...
        if isempty(time_data)
            # If there is no time-varying data available: Use the registered capacity
            # TODO: Update the correct column from the generator file
            gens_cap[i, :] = fill(round(Int,row[:totalcapacity]), units.N)
